...
function createBoard(board) {
    return __awaiter(this, void 0, void 0, function* () {
        if ((0, validators_1.isEmpty)(board.name)) {
            (0, common_1.throwNewError)(validators_1.VALIDATION_ERROR_NAME, 'Board name is required!');
        }
        if (isThereAnyEmptyColumn(board)) {
            (0, common_1.throwNewError)(validators_1.VALIDATION_ERROR_NAME, 'Column name is required!');
        }
        if (getDuplicatedColumns(board).length > 0) {
            const duplicatedColumns = getDuplicatedColumns(board);
            (0, common_1.throwNewError)(validators_1.VALIDATION_ERROR_NAME, `The column${duplicatedColumns.length > 1 ? 's' : ''} "${duplicatedColumns.join(', ')}" ${duplicatedColumns.length > 1 ? 'are' : 'is'} duplicated!`);
        }
        const boardFound = yield getDuplicatedBoard(board);
        if (!!boardFound) {
            (0, common_1.throwNewError)(validators_1.VALIDATION_ERROR_NAME, `There is already a board with the name "${boardFound.name}". Please choose a different one!`);
        }
